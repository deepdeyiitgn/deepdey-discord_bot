...
class Reminders(commands.Cog):
    ...
    @app_commands.command(name='remind', description='Create a reminder: /remind 10m Take a break')
    async def slash_remind(self, interaction: discord.Interaction, timestr: str, message: str):
        await interaction.response.defer()
        try:
            seconds = parse_time(timestr)
        except ValueError:
            await interaction.followup.send('Invalid time. Use formats like 10m, 2h, 1d, 30s', ephemeral=True)
            return
        when = datetime.datetime.utcnow() + datetime.timedelta(seconds=seconds)
        r = {
        ...
        self.reminders.append(r)
        await async_save_json(DATA_PATH, self.reminders)
        await interaction.followup.send(f'Reminder set for {timestr} from now.', ephemeral=True)
    ...
    async def slash_list_reminders(self, interaction: discord.Interaction):
        await interaction.response.defer()
        user = interaction.user.id
        items = [r for r in self.reminders if r['user_id'] == user]
        if not items:
            await interaction.followup.send('You have no reminders.', ephemeral=True)
            return
        out = []
        for i, r in enumerate(items, start=1):
            out.append(f"{i}. At {r['when']}: {r['message']}")
